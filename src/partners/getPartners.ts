...
export const getPartners = (app: Express, db: Pool) => {
 ...
 app.get('/partners', async (req, res) => {
  dotenv.config();
  const query = 'SELECT id, title, logo_file, date_last_edit FROM partners ORDER BY date_last_edit DESC';
  const result = await db.query(query);

  const partners = result.rows.map(partner => {
    const logoPath = `${process.env.BACKEND_CONNECTION}/${path.basename(partner.logo_file)}`;
